...
import (
	...
	"github.com/dapr/components-contrib/state/weakisolationmockdb"
...
func main() {
	...
	err = rt.Run(
		...
		runtime.WithStates(
			...
			state_loader.New("weakisolationmockdb", func() state.Store {
				return weakisolationmockdb.NewWeakIsolationMockDBStateStore(logContrib)
			}),
		),
		runtime.WithPubSubs(
