...
func (s *UserService) AddUser(user spec.User) error {
	...
	_, errorExists := JSONData["error"]

	// If we get any data, that wali maus means we found a user, that's an error in our case
	if len(data) > 0 && !errorExists {
		fmt.Printf("[USER::impl] non-zero data %s\n", data)
	...
	prob = s.SaveState(s.storeName, url.QueryEscape(user.Username), user)
	if prob != nil {
		return prob
	}
...
func (s *UserService) GetUser(username string) (*spec.User, error) {
	...
	fmt.Printf("[USER::impl] user data %s\n", data)
	user := &spec.User{}
