...
func (c *Cassandra) Delete(req *state.DeleteRequest) error {
	return c.session.Query("DELETE FROM dapr.items WHERE key = ?", req.Key).Exec()
}
...
func (c *Cassandra) Get(req *state.GetRequest) (*state.GetResponse, error) {
	...
	results, err := session.Query("SELECT value FROM dapr.items WHERE key = ?", req.Key).Iter().SliceMap()
	if err != nil {
		return nil, err
	}
...
func (c *Cassandra) Set(req *state.SetRequest) error {
	...
	return session.Query("INSERT INTO dapr.items (key, value) VALUES (?, ?)", req.Key, bt).Exec()
}
